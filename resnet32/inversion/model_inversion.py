...
class ModelInversionRunner(object):
    def __init__(self, generator_type, model, lr, train_steps, batch_size, tau, alpha_pr, alpha_rf,
        ...
        elif generator_type == 'seq_cifar100_large':
            self.generator = generators.CIFAR_GEN_large()
    ...
    def get_data(self, batch_size, return_loss=False, return_logit=False):
        with torch.no_grad():
            ...
            if return_loss:
                if return_logit:
                    return inputs, target, output, self.slt_loss(output, target)
                else:
                    return inputs, target, self.slt_loss(output, target)
            else:
                if return_logit:
