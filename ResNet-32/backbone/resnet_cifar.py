...
def get_all_blocks(resnet_model):
    """
    used for get all the res-block and first layer from resnet32 defined in this file.
    :param resnet_model:
    :return:
    """
    all_names = []
    for n, mi in resnet_model.named_modules():
        all_names.append(n)
    # get stages
    stages = []
    for n in all_names:
        for sn in n.split('.'):
            if 'stage' in sn and sn not in stages:
                stages.append(sn)
    # get blocks of each stage
    block0 = torch.nn.Sequential(resnet_model.conv_1_3x3, resnet_model.bn_1, torch.nn.ReLU())
    all_blocks = [block0]
    for si in stages:
        stage = getattr(resnet_model, si)
        if isinstance(stage, Stage):
            mlist = getattr(stage, 'blocks')
            use_relu = getattr(stage, 'block_relu')
            for bi in mlist:
                if use_relu or bi.last_relu:
                    all_blocks.append(torch.nn.Sequential(bi, torch.nn.ReLU()))
                else:
                    all_blocks.append(bi)
        elif isinstance(stage, ResidualBlock):
            all_blocks.append(stage)
    return all_blocks
