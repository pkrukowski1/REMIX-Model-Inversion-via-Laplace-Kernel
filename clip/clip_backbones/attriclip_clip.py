...
class PromptLearner(nn.Module):
    ...
    def text_prompts(self, x_query, l, known_classes=None, total_classes=None):
        ...
        if l in self.t_layers:
            ...
            K = getattr(self,f'text_k_{l}') # 0 based indexing here
            p = getattr(self,f'text_p_{l}') # 0 based indexing here
            ...
            if self.memory_size == 0:
                ...
                top_k = torch.topk(cos_sim, self.top_k, dim=0)
                k_idx = top_k.indices
                P_ = p[k_idx]
            print(k_idx.shape)
            print(f'P_ shape: {P_.shape}')
                
            # E_ = P_.reshape((-1,self.t_emb_d))
            # E_ = torch.mean(P_, dim=0)
            # E_ = P_.reshape(P_.shape[0], -1, P_.shape[-1])
            if l == 0:
                prefix = self.token_prefix
                suffix = self.token_suffix
                print(f'E_ shape: {E_.shape}')
                text_p = self.construct_prompts(E_, prefix, suffix)
...
class CustomCLIP(nn.Module):
    ...
    def forward(self, image, label=None, known_classes=None, total_classes=None, return_feat=False):
        ...
        prompts, prompt_loss = self.prompt_learner(x_query=image_features, l=0, known_classes=known_classes, total_classes=total_classes, text_layer=True)
...
def get_attriclip(cfg, n_classes):
