...
class PromptLearner(nn.Module):
    def __init__(self, args, v_emb_d=768, t_emb_d=512, key_dim=512, clip_model=None, tokenizer=None):
        ...
        for i in self.t_layers:
            if i == 0 and args['ctx_init'] and args['n_ctx_text'] <= 4:
        ...
        # visual prompt init
        # TODO: alternative, shared keys across layers
        ...
        #                 "language_ctx": 0}
        # clip_model_temp = create_model(args['backbone_type'], pretrained=args['pretrained_weight'], design_details = design_details)
        # # clip_model_temp_image = create_model(args['backbone_type'], pretrained=args['pretrained_weight'], design_details = design_details)
        # with torch.no_grad():
        #     # embedding = clip_model.token_embedding(tokenized_prompts).type(self.dtype)
        #     # self.ZS_image_encoder = clip_model_temp_image.visual
        #     # self.ZS_clip = clip_model_temp
        #     self.ZS_clip_encode_text = clip_model_temp.encode_text
    ...
    def _init_smart(self, args):
        ...
        # self.t_pool_size = 1
        self.t_pool_size = args['pool_size']
    ...
    def increment_class(self, classnames, known_classes=None, total_classes=None):
        ...
        # get text features of incoming classes
        # with torch.no_grad():
        #     # embedding = self.ZS_clip.token_embedding(tokenized_prompts).type(self.dtype)
        #     embedding = self.token_embedding(tokenized_prompts).type(self.dtype)
        #     template = 'a photo of a {}'
        #     x = [template.replace("{}", name) for name in classnames]
        #     # x_tokenized = torch.cat([clip.tokenize(p) for p in x])
        #     x_tokenized = torch.cat([self.tokenizer(p) for p in x])
        #     # text_features = self.ZS_clip.encode_text(x_tokenized.cuda())
        #     text_features = self.ZS_clip_encode_text(x_tokenized.cpu())
        #         # all_teacher_features.append(text_features.unsqueeze(1))
        
        
        # # init the keys of new incoming classes with text features
        # for i in self.v_layers:
        #     print(f"init the layer {i} keys of new incoming classes with text features")
        #     K = getattr(self,f'visual_k_{i}').clone()
        #     # print(K.shape)
        #     # print(text_features.shape)
        #     K[known_classes:total_classes, :] = text_features[known_classes:total_classes, :]
        #     setattr(self, f'visual_k_{i}', torch.nn.Parameter(K))
    ...
    def text_prompts(self, x_query, l, known_classes=None, total_classes=None):
        ...
        if l in self.t_layers:
            ...
            w = q@n_K.T
            # temp=0.01
            # w = torch.exp(q@n_K.T/temp)
            ###############################################
            w = nn.functional.normalize(w, p=1, dim=-1)
            w = torch.sum(w.T, dim=1)
            text_p = w.unsqueeze(1).unsqueeze(1) * p 

            if l == 0:
                prefix = self.token_prefix
                suffix = self.token_suffix
                text_p = self.construct_prompts(text_p, prefix, suffix)
...
class TextEncoder(nn.Module):
    ...
    def forward(self, prompts, tokenized_prompts, x_query=None, prompt_learner=None, known_classes=None, total_classes=None):
        ...
        x = self.transformer(x, x_query=x_query, prompt_learner=prompt_learner, known_classes=known_classes, total_classes=total_classes)
...
class CustomCLIP(nn.Module):
    ...
    def encode_image(self, image, x_query=None, known_classes=None, total_classes=None):
    ...
    def encode_text(self, prompts, tokenized_prompts, x_query=None, known_classes=None, total_classes=None):
        assert x_query is not None
        return self.text_encoder(prompts, tokenized_prompts, x_query=x_query, prompt_learner=self.prompt_learner, known_classes=known_classes, total_classes=total_classes)
    ...
    def forward(self, image, label=None, known_classes=None, total_classes=None):
        ...
        x_query = self.image_encoder(image)
        prompts = self.prompt_learner(x_query=x_query, l=0, known_classes=known_classes, total_classes=total_classes, text_layer=True)
        # Compute the prompted image and text features
        # text_features = self.text_encoder(prompts, tokenized_prompts)
        # image_features = self.image_encoder(image.type(self.dtype))
        text_features = self.encode_text(prompts, tokenized_prompts, x_query=x_query, known_classes=known_classes, total_classes=total_classes)
        image_features = self.encode_image(image.type(self.dtype), x_query=x_query, known_classes=known_classes, total_classes=total_classes)
...
def get_codavlpt(cfg, n_classes):
