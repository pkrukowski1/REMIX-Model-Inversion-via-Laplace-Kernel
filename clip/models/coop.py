...
from clip_backbones.coop_clip import get_coop
...
class Learner(BaseLearner):
    def __init__(self, args):
        ...
        self._network, self.train_trfm, self.test_trfm = get_coop(args, [])
    ...
    def _init_train(self, train_loader, test_loader, optimizer, scheduler, logger=None):
        ...
        for _, epoch in enumerate(prog_bar):
            ...
            for iteration, (_, inputs, targets) in enumerate(tqdm(train_loader), start=self._cur_task*itr_per_task + len(train_loader)*epoch):
                scheduler(iteration)
