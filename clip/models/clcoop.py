...
from clip_backbones.clcoop_clip import get_clcoop
...
class Learner(BaseLearner):
    def __init__(self, args):
        ...
        self._network, self.train_trfm, self.test_trfm = get_clcoop(args, [])
    ...
    def _init_train(self, train_loader, test_loader, optimizer, scheduler, logger=None):
        ...
        for _, epoch in enumerate(prog_bar):
            ...
            for iteration, (_, inputs, targets) in enumerate(train_loader, start=len(train_loader)*epoch):
                ...
                # text_self_sim = f_t @ f_t.t()
                # t_labels = torch.arange(f_t.shape[0]).unsqueeze(0)
                # mask = 1 - torch.eq(t_labels, t_labels.t()).float()
                # # print(mask, text_self_sim)
                # text_self_sim = mask.cuda() * text_self_sim
                # text_self_sim = text_self_sim[self._known_classes:, :]
                # loss_text_div = max(torch.mean(torch.sum(text_self_sim, dim=1)), 0)
                
                # loss_text_div = ((f_t @ f_t.T - torch.eye(f_t.shape[0]).cuda())**2).mean()
                
                # loss = loss_ce_i + loss_text_div
                ...
                if logger:
                    ...
                    # logger.add_scalar("loss/text_div", loss_text_div, iteration + self._cur_task * total_iterations)
