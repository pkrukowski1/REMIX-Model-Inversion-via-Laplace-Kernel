...
# from clip_backbones.vl_l2p import PromptVLNet
from clip_backbones.vit_l2p import get_cl_l2p
...
class Learner(BaseLearner):
    def __init__(self, args):
        ...
        #     self._network = PromptVLNet(args, True)
        # else:
        # self._network = PromptVitNet(args, True)
        self._network, self.train_trfm, self.test_trfm = get_cl_l2p(args, args['n_classes'])
    ...
    def _init_train(self, train_loader, test_loader, optimizer, scheduler, logger=None):
        ...
        for _, epoch in enumerate(prog_bar):
            ...
            for iteration, (_, inputs, targets) in enumerate(train_loader, start=len(train_loader)*epoch):
                ...
                output, prompt_loss = self._network(inputs, task_id=self._cur_task, train=True)
                ...
                loss = loss_ce + 0.1*prompt_loss.sum()
    ...
    def _extract_vectors(self, loader):
        ...
        with torch.no_grad():
            for _, _inputs, _targets in loader:
                ...
                _vectors = tensor2numpy(_vectors[:, :25, :].mean(dim=1).view(_vectors.size(0), -1))
