...
############################################################################################
@register_model
def vit_base_patch16_clip_224_l2p(pretrained=False, **kwargs):
    """ ViT-Base (ViT-B/16) from original paper (https://arxiv.org/abs/2010.11929).
    A Vision Transformer (ViT) image classification model. Pretrained on LAION-2B image-text pairs using 
    OpenCLIP. Fine-tuned on ImageNet-12k and then ImageNet-1k in timm.
    """
    model_kwargs = dict(patch_size=16, embed_dim=768, depth=12, num_heads=12, **kwargs)
    # model = _create_vision_transformer('vit_base_patch16_clip_224.laion2b_ft_in12k_in1k', pretrained=pretrained, **model_kwargs)
    # model = _create_vision_transformer('vit_base_patch16_clip_224.openai', pretrained=True, **model_kwargs)
    model = _create_vision_transformer('vit_base_patch16_clip_224.laion2b', pretrained=True, **model_kwargs)
    return model
