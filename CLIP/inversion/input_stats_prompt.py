...
def get_transformer_block_stats(model, samples, img_size, out_path, on_cuda):
    ...
    # register hooks:
    stat_hooks = []
    for n, mi in model.clip_model.visual.named_modules():
...
def get_block_input_stats(model, samples, img_size, out_path, on_cuda, split_cnn=False):
    all_blocks, batch_dims, use_prompt = utils.split_clip_blocks(
        model=model.clip_model.visual, normalize=True, split_cnn=split_cnn)
    # add block hooks
    bid2hookds = {}
    for i, bi in enumerate(all_blocks):
        if i == 0:
            continue
        elif split_cnn and i == 1:
            bid2hookds[i] = [
                InputStatHook(module_name='total', module=all_blocks[0], batch_dim=batch_dims[1], is_input=False)]
        elif i == len(all_blocks) - 1:
            bid2hookds[i] = [
                InputStatHook(module_name='total', module=all_blocks[i - 1],
                              batch_dim=batch_dims[i], is_input=False)
            ]
        else:  # for transformer blocks
...
def get_data_block_input_stats(model, data_loader, out_path, on_cuda, fname='block_input_stats', split_cnn=False):
    all_blocks, batch_dims = utils.split_clip_blocks(model=model.clip_model.visual, normalize=True, split_cnn=split_cnn)
    # add block hooks
    bid2hookds = {}
    for i, bi in enumerate(all_blocks):
        if i == 0:
...
class InputStatHook(object):
    def __init__(self, module, module_name, batch_dim, is_input=True):
        self.module = module
    ...
    def update_stat(self):
        if self.is_input:  # get stats of input
            ...
            self.cnt += batch_size
        else:  # get stats of output
            batch_size = self.outputs.shape[0]
            if len(self.outputs.shape) == 4:  # consider 4-dim input
                mean = self.outputs.mean([self.batch_dim, 2, 3]).detach().cpu()
                mean_square = (self.outputs ** 2).mean([self.batch_dim, 2, 3]).detach().cpu()
            else:
                mean = self.outputs.mean([self.batch_dim, 2]).detach().cpu()
                mean_square = (self.outputs ** 2).mean([self.batch_dim, 2]).detach().cpu()
            self.mean = self.cnt / (self.cnt + batch_size) * self.mean + batch_size / (self.cnt + batch_size) * mean
            self.mean_square = self.cnt / (self.cnt + batch_size) * self.mean_square + \
                               batch_size / (self.cnt + batch_size) * mean_square
            self.cnt += batch_size
