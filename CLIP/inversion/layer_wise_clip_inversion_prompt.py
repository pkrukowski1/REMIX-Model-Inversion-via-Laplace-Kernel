...
from inversion import input_stats_prompt
...
class LayerWiseCLIPInversion(object):
    ...
    def get_input_stat(self, model):
        # prepare input stats
        input_stats_prompt.get_transformer_block_stats(
            model=model, samples=10000, img_size=[3, self.input_resolution, self.input_resolution],
            out_path=self.local_path, on_cuda=self.on_cuda
        )
        input_stats_prompt.get_block_input_stats(
            model=model, samples=10000, img_size=[3, self.input_resolution, self.input_resolution],
            out_path=self.local_path, on_cuda=self.on_cuda, split_cnn=False if self.start_block == 0 else True
        )

    def update_input_stat(self, data_loader, model):
        ...
        if not os.path.exists(ori_model_file) and len(ori_block_files) == 0:
            input_stats_prompt.get_data_transformer_block_stats(
                model=model, data_loader=data_loader, out_path=self.local_path,
                on_cuda=self.on_cuda, fname='model_input_stats.pkl'
            )
            input_stats_prompt.get_data_block_input_stats(
                model=model, data_loader=data_loader, out_path=self.local_path,
            ...
            # merge stats
            input_stats_prompt.merge_stats(
    ...
    def inversion(self, inputs, target_feats, query, opt_param, loss_fn, model, size_change=None, prompt=None,
                  is_input=False, iters=None, norm_feat=True, return_best=False, input_loss=None, alpha_feat=None,
                  l_pr=None, in_aug=None, verbose=True, id_bias=None, full_model=True):
        best_loss = None
        best_inputs = None
        if iters is None:
        ...
        for i in range(iters):
            ...
            if is_input:
                ...
                aug_inputs = inputs
            if full_model:
                out_feat = model(aug_inputs, x_query=query, prompt_learner=prompt)[0]
            else:
    ...
    def layer_wise_inversion_for_cl(self, batch_size, target_feats, query, finetune_iters, finetune_lr, milestones,
                                    prompt, iters=None, size_change=None, return_best=True, first_layer_param=None,
        ...
        # get all blocks
        all_blocks, batch_dims, use_prompt = utils.split_vit_blocks_prompt(
            model=self.model, normalize=self.normalize, prompt=prompt,
            split_cnn=False if self.start_block == 0 else True
        )
        ...
        for i in range(steps):
            ...
            for j, bi in enumerate(all_blocks):
                if use_prompt[j]:
                    bi.set_query(query=query[start:end, :])
            self.register_hooks()
            # add choice of downscale image and gradual increase size
            opt_inputs, best_loss = self.inversion(
                inputs=inputs, target_feats=target_feats[start:end, :], query=query,
                opt_param=opt_param, size_change=size_change,
                norm_feat=self.normalize, is_input=True, prompt=prompt,
