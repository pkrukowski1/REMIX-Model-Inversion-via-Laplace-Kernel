...
from clip_backbones.textf_prcil_clip import get_vl_prcil
    ...
    def _init_train_text(self, train_loader, test_loader, optimizer, scheduler, logger=None):
        ...
        for _, epoch in enumerate(prog_bar):
            ...
            for iteration, (_, inputs, targets) in enumerate(train_loader, start=len(train_loader)*epoch):
                ...
                f_i, f_t, scale, prompt_loss = self._network.forward_text(inputs, known_classes=self._known_classes, total_classes=self._total_classes)
                ############################################################
                # CE loss
                ...
                # loss_t_kd = F.mse_loss(f_t, self.text_features)
                loss_t_kd = torch.norm(f_t - self.text_features, dim=-1, p=2).pow(2.0).mean()
                                
                # loss = (loss_ce_i + loss_ce_t + prompt_loss)
                # loss = (loss_ce_i + loss_ce_t + loss_t_div )
                loss = (loss_ce_i + loss_ce_t + loss_t_div + loss_t_kd)
                ...
                if logger:
                    ...
                    logger.add_scalar("loss_text/t_div", loss_t_div, iteration + self._cur_task * total_iterations)
                    logger.add_scalar("loss_text/t_kd", loss_t_kd, iteration + self._cur_task * total_iterations)
                    logger.add_scalar("loss_text/prompt_loss", prompt_loss, iteration + self._cur_task * total_iterations)
                    logger.add_scalar('Lr/text', optimizer.param_groups[0]['lr'], iteration + self._cur_task * total_iterations)
    ...
    def _init_train_image(self, train_loader, test_loader, optimizer, scheduler, logger=None):
        ...
        for _, epoch in enumerate(prog_bar):
            ...
            for iteration, (_, inputs, targets) in enumerate(train_loader, start=len(train_loader)*epoch):
                ...
                f_i, f_t, scale, prompt_loss = self._network.forward_image(inputs, known_classes=self._known_classes, total_classes=self._total_classes)                
                ############################################################
                # CE loss
                ...
                if logger:
                    ...
                    logger.add_scalar('Lr/img', optimizer.param_groups[0]['lr'], iteration + self._cur_task * total_iterations)
                ############################################################
