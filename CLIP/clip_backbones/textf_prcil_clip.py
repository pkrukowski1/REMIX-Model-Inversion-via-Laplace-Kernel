...
class PromptLearner(nn.Module):
    ...
    def text_prompts(self, x_query, l, known_classes=None, total_classes=None):
        ...
        if l in self.t_layers:
            ...
            if self.training:
                loss = 0
                loss += ortho_penalty(p.view(p.shape[0], -1)) 
            else:
                loss = 0
        else:
            loss = 0
...
class TextEncoder(nn.Module):
    ...
    def forward(self, prompts, tokenized_prompts, x_query=None, prompt_learner=None, known_classes=None, total_classes=None, prompt_loss=None):
        x = prompts + self.positional_embedding.type(self.dtype)
        x = x.permute(1, 0, 2)  # NLD -> LND
        x, prompt_loss = self.transformer(x, x_query=x_query, prompt_learner=prompt_learner, known_classes=known_classes, total_classes=total_classes, prompt_loss=prompt_loss)
...
class CustomCLIP(nn.Module):
    ...
    def encode_text(self, prompts, tokenized_prompts, known_classes=None, total_classes=None, prompt_loss=None):
        return self.text_encoder(prompts, tokenized_prompts, prompt_learner=self.prompt_learner, known_classes=known_classes, total_classes=total_classes, prompt_loss=prompt_loss)
    ...
    def forward(self, image, label=None, known_classes=None, total_classes=None):
        ...
        prompts, _ = self.prompt_learner(x_query=None, l=0, known_classes=known_classes, total_classes=total_classes, text_layer=True)
        # Compute the prompted image and text features
        # text_features = self.text_encoder(prompts, tokenized_prompts)
        # image_features = self.image_encoder(image.type(self.dtype))
        text_features, _ = self.encode_text(prompts, tokenized_prompts, known_classes=known_classes, total_classes=total_classes)
        image_features, _ = self.encode_image(image.type(self.dtype), known_classes=known_classes, total_classes=total_classes)
    ...
    def forward_text(self, image, label=None, known_classes=None, total_classes=None):
        tokenized_prompts = self.tokenized_prompts
        logit_scale = self.logit_scale.exp()

        prompts, prompt_loss_0 = self.prompt_learner(x_query=None, l=0, known_classes=known_classes, total_classes=total_classes, text_layer=True)
        # Compute the prompted image and text features
        # text_features = self.text_encoder(prompts, tokenized_prompts)
        # image_features = self.image_encoder(image.type(self.dtype))
        text_features, prompt_loss = self.encode_text(prompts, tokenized_prompts, known_classes=known_classes, total_classes=total_classes, prompt_loss=prompt_loss_0)
    ...
    def forward_image(self, image, label=None, known_classes=None, total_classes=None):
        tokenized_prompts = self.tokenized_prompts
        logit_scale = self.logit_scale.exp()
